...
def make_dls(stim_path, batch_sz=24, fov_noise=False, seed=0, test_prop=0.2):
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    for (inputs, labels) in train_loader:
                        ...
                        inputs = (inputs[0], 
                                  inputs[1], 
                                  inputs[2] + v * torch.randn(inputs[2].shape, 
                                                          device='cuda'))
                    ...
                    log_msg = f"%5d / %5d TRAIN/TEST losses: \t \t \t\t acc: \t %.2f %% \t %.2f %% \t\t time: {round(end,3)}" % (
                        cycle + 1,
                        epoch + 1,
                        te_running_loss,
                        100 * te_correct / te_total,
...
def inspect_results_test(res):
    ...
    sn.violinplot(data=res_test_1, x='noise_var', y='te_acc', ax=ax[0, 0])
    sn.violinplot(data=res_test_2, x='noise_var', y='te_acc', ax=ax[0, 1])
    sn.violinplot(data=res_test_3, x='noise_var', y='te_acc', ax=ax[0, 2])
    ...
    # ax[0, 0].set_xticks(rotation='vertical')
    # ax[0, 1].set_xticks(rotation='vertical')
    # ax[0, 2].set_xticks(rotation='vertical')
    plt.tight_layout()
    plt.show()
...
def inspect_results(res):
    ...
    sn.heatmap(df_cm, annot=True, cbar=False, cmap="Blues", fmt="d", ax=ax[0, 0])
...
def train_nets(p):
    ...
    dls = make_dls(stim_path, batch_sz, fov_noise, seed)
    train_loader = dls.train
    test_loader = dls.valid
    # dls.show_batch(max_n = 2)
    # plt.show()
    
    net_1 = SiameseNet(w_dropout_1, w_dropout_2, 1)
    net_2 = SiameseNet(w_dropout_1, w_dropout_2, 2)
    net_3 = SiameseNet(w_dropout_1, w_dropout_2, 3)
    
    net_1 = init_weights(net_1)
    net_2 = init_weights(net_2)
    net_3 = init_weights(net_3)
    
    net_1 = init_pretrained_weights(net_1)
    net_2 = init_pretrained_weights(net_2)
    net_3 = init_pretrained_weights(net_3)
    
    criterion = nn.CrossEntropyLoss()
    
    params_to_update_1 = net_1.parameters()
    params_to_update_2 = net_2.parameters()
    params_to_update_3 = net_3.parameters()
    
    optimizer_1 = optim.Adam(filter(lambda p: p.requires_grad, params_to_update_1),
                             lr=lr_min,
                             weight_decay=weight_decay)
    optimizer_2 = optim.Adam(filter(lambda p: p.requires_grad, params_to_update_2),
                             lr=lr_min,
                             weight_decay=weight_decay)
    optimizer_3 = optim.Adam(filter(lambda p: p.requires_grad, params_to_update_3),
                             lr=lr_min,
                             weight_decay=weight_decay)
    
    net_1 = nn.DataParallel(net_1)
    net_2 = nn.DataParallel(net_2)
    net_3 = nn.DataParallel(net_3)
    
    net_1.to('cuda')
    net_2.to('cuda')
    net_3.to('cuda')
    
    p_1 = (net_1, optimizer_1, criterion, cycles, epochs, train_loader,
           test_loader)
    p_2 = (net_2, optimizer_2, criterion, cycles, epochs, train_loader,
           test_loader)
    p_3 = (net_3, optimizer_3, criterion, cycles, epochs, train_loader,
           test_loader)
    ...
    torch.save(net_3.state_dict(), 'net_3.pth')
    
    inspect_results(res_1)
    inspect_results(res_2)
    inspect_results(res_3)
...
def train_nets_2(p):
    ...
    net_1 = SiameseNet(w_dropout_1, w_dropout_2, 12)
    net_2 = SiameseNet(w_dropout_1, w_dropout_2, 22)
    net_3 = SiameseNet(w_dropout_1, w_dropout_2, 32)
    ...
    torch.save(net_1.state_dict(), 'net_12.pth')
    torch.save(net_2.state_dict(), 'net_22.pth')
    torch.save(net_3.state_dict(), 'net_32.pth')
...
def test_nets_noise():
    ...
    net_1.load_state_dict(torch.load('net_1.pth'))
    net_2.load_state_dict(torch.load('net_2.pth'))
    net_3.load_state_dict(torch.load('net_3.pth'))
    ...
    p_1 = (net_1, optimizer_1, criterion, cycles, epochs, train_loader,
           test_loader, noise_levels)
    p_2 = (net_2, optimizer_2, criterion, cycles, epochs, train_loader,
           test_loader, noise_levels)
    p_3 = (net_3, optimizer_3, criterion, cycles, epochs, train_loader,
           test_loader, noise_levels)
    
    res_test_1 = test_net(p_1)
    res_test_2 = test_net(p_2)
    res_test_3 = test_net(p_3)
    
    res = (res_test_1, res_test_2, res_test_3)
    inspect_results_test(res)
    
def test_nets_noise_2():
    ...
    net_1.load_state_dict(torch.load('net_12.pth'))
    net_2.load_state_dict(torch.load('net_22.pth'))
    net_3.load_state_dict(torch.load('net_32.pth'))
